...
class APICalls:
    ...
    def _add_domain(my_headers, domain_name, account_id):
        ...
        if response["success"] == False:
            ...
            self.flare.issue_log.append(f"API call to DNS rule for {domain_name} failed.")
    ...
    def _create_dns_record(my_headers, domain_name, content, zone_identifier):
        ...
        if response["success"] == False:
            ...
            self.flare.issue_log.append(f"API call to create DNS rule for {domain_name} failed.")
            self.settings.APIfalures -= 1
    ...
    def _create_page_rule():
        ...
        if response["success"] == False:
            print(f"[WARNING] API call to add a page rule for zone {zone_identifier} failed.")
            self.settings.APIfalures -= 1
            self.flare.issue_log.append(f"API call to create page rule for {zone_identifier} failed.")
    ...
    def _response_code_handler(response):
        if response.status_code == 400:
            print("Oops, 400 error, bad request. Change request format before trying again. Exiting...")
            sys.exit()
        if response.status_code == 401:
            print("Oops, 401 error, unauthorized. Check your API token, if repeated errors check header format. Exiting...")
            sys.exit()
        if response.status_code == 403:
            print("Oops, 403 error, forbidden. Check your API token. If repeated errors with a valid API token, check API token permissions.")
            sys.exit()
